...
public class HumanoidRotataionSetter : MonoBehaviour
{
    ...
    private void UpdateRotationX(float value)
    {
        ...
        {
            Vector3 modelRot = _rotationHumanoid.transform.rotation.eulerAngles;

            _rotationHumanoid.transform.rotation = Quaternion.Euler(value, modelRot.y, modelRot.z);
            ...
            EditManager.GetInstance().ChangeEntireRot(frame, _rotationHumanoid.transform.rotation);
        }
    }
    ...
    private void UpdateRotationY(float value)
    {
        if (_animationTransform != null && frame != 0 && frame != EditManager.GetInstance().KeyPoseList.Count - 1)
        {
            ...
            _rotationHumanoid.transform.rotation = Quaternion.Euler(modelRot.x, -value, modelRot.z);
            Vector3[] positionValue = _animationTransform.ReturnNewPositionValue();
            Quaternion[] rotationValue = _animationTransform.ReturnNewRotationValue();
    ...
    private void UpdateRotationZ(float value)
    {
        if (_animationTransform != null && frame != 0 && frame != EditManager.GetInstance().KeyPoseList.Count - 1)
        {
            ...
            _rotationHumanoid.transform.rotation = Quaternion.Euler(modelRot.x, modelRot.y, value);
            Vector3[] positionValue = _animationTransform.ReturnNewPositionValue();
            Quaternion[] rotationValue = _animationTransform.ReturnNewRotationValue();
